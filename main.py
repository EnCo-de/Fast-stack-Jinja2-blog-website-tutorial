...
async def home(request: Request, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    return templates.TemplateResponse(request, "index.html", {"posts": posts, "title": "Posts"})
...
async def user_posts_page(
    ...
    stmt = select(models.Post).options(selectinload(models.Post.user)).where(models.Post.user_id == user_id)
    result = await db.execute(stmt)
    posts = result.scalars().all()
    return templates.TemplateResponse(
...
async def update_post_full(post_id: int, post_data: PostCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if post_data.user_id != post.user_id:
        stmt = select(models.User).where(models.User.id == post_data.user_id)
        result = await db.execute(stmt,)
        user = result.scalars().first()
        if not user:
            raise HTTPException(
